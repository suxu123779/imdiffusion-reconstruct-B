...
class TrainData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # RECON_CHANGE: 重构训练不再依赖训练阶段的人造插补 mask，但保留 gt_mask 字段以兼容现有 batch 接口。
        ...
        return {
            ...
            # RECON_CHANGE: 重构训练不再使用 strategy_type 控制插补模式，这里仅为兼容保留。
...
def get_dataloader(train_path, test_path, label_path,batch_size = 32,window_split=1,split=4,mask_ratio=0.5):
    ...
    test_data_strategy_1 = TestData(test_path, label_path, train_path,window_split=window_split,strategy=0,split=split,mask_list=None)
    test_data_strategy_2 = TestData(test_path, label_path, train_path, window_split=window_split, strategy=1,split=split,mask_list=None)
