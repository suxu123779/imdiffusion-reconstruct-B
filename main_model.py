...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device,ratio = 0.7):
        ...
        # RECON_CHANGE: 通过 task_mode 在不改 diffusion 主体的前提下切换训练目标，默认兼容旧的插补训练。
        self.task_mode = config["model"].get("task_mode", "imputation")
    ...
    def calc_loss(
        ...
        if self.task_mode == "reconstruction":
            # RECON_CHANGE: 重构训练对完整序列计算 loss，不再只监督被 mask 的位置。
    ...
    def reconstruct(self, observed_data, cond_mask, side_info, n_samples, strategy_type):
        B, K, L = observed_data.shape
        reconstructed_samples = torch.zeros(B, n_samples, K, L).to(self.device)
        last_step = self.num_steps - 1

        for i in range(n_samples):
            # RECON_CHANGE: 重构推理从完整序列的高噪声版本开始，再逐步反推回整段 reconstruction。
            noise = torch.randn_like(observed_data)
            current_alpha = self.alpha_torch[last_step]
            current_sample = (current_alpha ** 0.5) * observed_data + (1.0 - current_alpha) ** 0.5 * noise
            ...
            for t in range(last_step, -1, -1):
                ...
                if t > 0:
                    previous_alpha = self.alpha_torch[t - 1]
                    # RECON_CHANGE: reconstruction 推理不再做插补条件混合，而是用确定性反推得到下一步样本。
    ...
    def get_middle_reconstruct_value(
        ...
        for i in range(n_samples):
            ...
            for t in range(last_step, -1, -1):
                ...
                step_alpha = self.alpha_torch[t]
                reconstructed = (current_sample - (1.0 - step_alpha) ** 0.5 * predicted) / (step_alpha ** 0.5)
                reconstructed_middle_samples[:, t] = reconstructed.detach()
    ...
    def forward(self, batch, is_train=1):
        ...
        # print(observed_tp)
        if self.task_mode == "reconstruction":
            # RECON_CHANGE: 重构训练不再对 observed_data 二次挖空，cond_mask 仅保留为全零兼容占位。
            cond_mask = torch.zeros_like(observed_mask)
            # RECON_CHANGE: strategy_type 原本只服务于交替插补模式，重构训练固定为单一占位类别。
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            if self.task_mode == "reconstruction":
                # RECON_CHANGE: 重构推理不再依赖 gt_mask/target_mask，改为对完整序列输出 reconstruction。
                cond_mask = torch.zeros_like(observed_mask)
                strategy_type = torch.zeros_like(strategy_type)
                eval_points = observed_mask.clone()
                side_info = self.get_side_info(observed_tp, cond_mask)
                samples = self.reconstruct(observed_data, cond_mask, side_info, n_samples, strategy_type)

                for i in range(len(cut_length)):  # to avoid double evaluation
                    eval_points[i, ..., 0 : cut_length[i].item()] = 0
                return samples, observed_data, eval_points, observed_mask, observed_tp
    ...
    def get_middle_evaluate(self, batch, n_samples, return_pathB=False, pathB_mid_step=None):
        ...
        with torch.no_grad():
            if self.task_mode == "reconstruction":
                # RECON_CHANGE: reconstruction 中间结果沿用旧接口返回，便于后续 .pk 结构兼容。
                cond_mask = torch.zeros_like(observed_mask)
                strategy_type = torch.zeros_like(strategy_type)
                eval_points = observed_mask.clone()
                side_info = self.get_side_info(observed_tp, cond_mask)
