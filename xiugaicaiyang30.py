...
def merge(pkl_path, data_id, machine_number=""):
    ...
    head_middle = result[-1]      # 期望最终为 [diffusion_steps, head_len, feature]
    all_gen_middle = result[-2]   # 期望为 [batch, diffusion_steps, window_len, feature] 或类似
    ...
    all_gen = to_cpu_tensor(all_gen)
    all_target = to_cpu_tensor(all_target)
    head = to_cpu_tensor(head)
    head_target = to_cpu_tensor(head_target)
    head_middle = to_cpu_tensor(head_middle)
    all_gen_middle = to_cpu_tensor(all_gen_middle)

    print(f"[DEBUG] raw all_gen shape: {all_gen.shape}")
    print(f"[DEBUG] raw all_target shape: {all_target.shape}")
    print(f"[DEBUG] raw head shape: {head.shape}")
    print(f"[DEBUG] raw head_target shape: {head_target.shape}")
    print(f"[DEBUG] raw head_middle shape: {head_middle.shape}")
    print(f"[DEBUG] raw all_gen_middle shape: {all_gen_middle.shape}")

    # =========================
    # [修改点 2] 兼容 nsample=30 后的 head
    # 目标：head 必须最终变成 [head_len, feature]
    # =========================
    if head.dim() == 3:
        # 例如 [nsample, head_len, feature] -> [head_len, feature]
        head = head.median(dim=0).values
    elif head.dim() == 2:
        pass
    else:
        raise ValueError(f"Unexpected head shape: {head.shape}")

    # =========================
    # [修改点 3] 兼容 head_target
    # 目标：head_target 最终也是 [head_len, feature]
    # =========================
    if head_target.dim() == 3:
        head_target = head_target.median(dim=0).values
    elif head_target.dim() == 2:
        pass
    else:
        raise ValueError(f"Unexpected head_target shape: {head_target.shape}")

    # =========================
    # [修改点 4] 兼容 head_middle
    # 目标：head_middle 最终为 [diffusion_steps, head_len, feature]
    # =========================
    if head_middle.dim() == 4:
        # 常见情况：[1, diffusion_steps, head_len, feature]
        head_middle = head_middle.squeeze(0)
    elif head_middle.dim() == 3:
        pass
    else:
        raise ValueError(f"Unexpected head_middle shape: {head_middle.shape}")

    # =========================
    # [修改点 5] 兼容 all_gen
    # 目标：all_gen 最终变成 [N, feature]
    # 旧逻辑假设它是 [batch, win_len, feature]
    # =========================
    if all_gen.dim() == 3:
        feature_number = all_gen.shape[-1]
        all_gen = all_gen.reshape(-1, feature_number)
    elif all_gen.dim() == 2:
        feature_number = all_gen.shape[-1]
    else:
        raise ValueError(f"Unexpected all_gen shape: {all_gen.shape}")

    # =========================
    # [修改点 6] 兼容 all_target
    # 目标：all_target 最终变成 [N, feature]
    # =========================
    if all_target.dim() == 3:
        all_target = all_target.reshape(-1, feature_number)
    elif all_target.dim() == 2:
        pass
    else:
        raise ValueError(f"Unexpected all_target shape: {all_target.shape}")

    # =========================
    # [修改点 7] 兼容 all_gen_middle
    # 目标：最终变成 [diffusion_steps, N, feature]
    # 旧逻辑默认输入是 [batch, diffusion_steps, win_len, feature]
    # =========================
    if all_gen_middle.dim() == 4:
        # 假设当前是 [batch, diffusion_steps, win_len, feature]
        all_gen_middle = all_gen_middle.permute(1, 0, 2, 3)  # -> [diffusion_steps, batch, win_len, feature]
        diffusion_steps = all_gen_middle.shape[0]
        all_gen_middle = all_gen_middle.reshape(diffusion_steps, -1, feature_number)
    elif all_gen_middle.dim() == 3:
        # 已经可能是 [diffusion_steps, N, feature]
        diffusion_steps = all_gen_middle.shape[0]
    else:
        raise ValueError(f"Unexpected all_gen_middle shape: {all_gen_middle.shape}")

    print(f"[DEBUG] fixed head shape: {head.shape}")
    print(f"[DEBUG] fixed head_target shape: {head_target.shape}")
    print(f"[DEBUG] fixed head_middle shape: {head_middle.shape}")
    print(f"[DEBUG] fixed all_gen shape: {all_gen.shape}")
    print(f"[DEBUG] fixed all_target shape: {all_target.shape}")
    print(f"[DEBUG] fixed all_gen_middle shape: {all_gen_middle.shape}")

    # =========================
    # [修改点 8] 拼接前再做一次维度检查
    # =========================
    if head.dim() != all_gen.dim():
        raise ValueError(f"head dim {head.dim()} != all_gen dim {all_gen.dim()}, head={head.shape}, all_gen={all_gen.shape}")

    if head_target.dim() != all_target.dim():
        raise ValueError(f"head_target dim {head_target.dim()} != all_target dim {all_target.dim()}, head_target={head_target.shape}, all_target={all_target.shape}")

    if head_middle.dim() != all_gen_middle.dim():
        raise ValueError(f"head_middle dim {head_middle.dim()} != all_gen_middle dim {all_gen_middle.dim()}, head_middle={head_middle.shape}, all_gen_middle={all_gen_middle.shape}")
    ...
    # =========================
    # 读取 label / origin_data
    # =========================
    if data_id == "SMD" or data_id == "GCP":
        print(f"machine number is {machine_number}")
        label = pickle.load(open(f"data/Machine/{machine_number}_test_label.pkl", "rb"))
        origin_data = pickle.load(open(f"data/Machine/{machine_number}_test.pkl", "rb"))
    else:
        label = pickle.load(open(f"data/Machine/{data_id}_test_label.pkl", "rb"))
        origin_data = pickle.load(open(f"data/Machine/{data_id}_test.pkl", "rb"))

    # =========================
    # [修改点 9] check equal 前先转 Tensor
    # =========================
    origin_data = torch.Tensor(origin_data)
    label = torch.Tensor(label)

    print(f"check equal is {torch.all(all_target == origin_data[:all_target.shape[0]] * 20)}")
