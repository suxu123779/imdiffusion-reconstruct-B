...
def load_score_run_dict(ensemble_dir, score_name):
    ...
    score_runs = {}
    for path in paths:
        score_runs[infer_run_id(path, score_name)] = tensor_to_numpy(
...
def rank_score_runs(score_runs):
    return {
        run_id: percentile_ranks(score)
        for run_id, score in score_runs.items()
    }
...
def aligned_len(labels, *scores):
    return min([len(labels), *[len(score) for score in scores]])
...
def main():
    ...
    parser.add_argument("--out", type=str, default="hidden_topology_auc_summary.csv")
    ...
    for variant in args.variants:
        ...
        final_rank_runs = rank_score_runs(final_runs)
        ...
        for score_name in args.score_names:
            ...
            pathB_raw_ensemble = mean_score_runs(pathB_runs, score_name)
            pathB_rank_runs = rank_score_runs(pathB_runs)
            pathB_rank_ensemble = mean_score_runs(pathB_rank_runs, f"{score_name}_rank")

            used_len = aligned_len(
                labels,
                final_raw_ensemble,
                final_rank_ensemble,
                pathB_raw_ensemble,
            ...
            pathB_raw_used = pathB_raw_ensemble[:used_len]
            pathB_rank_used = pathB_rank_ensemble[:used_len]
            label_sum_used = int(np.sum(label_used))

            if used_len != len(labels) or used_len != len(pathB_raw_ensemble):
                print(
                    ...
                    f"pathB_raw={len(pathB_raw_ensemble)}, "
                    f"pathB_rank={len(pathB_rank_ensemble)}; using {used_len}"
                )
            if label_sum_used == 0:
            ...
            auc_pathB_raw = roc_auc_score(label_used, pathB_raw_used)
            auc_pathB_rank = roc_auc_score(label_used, pathB_rank_used)
            auc_reverse = roc_auc_score(label_used, -pathB_rank_used)
            summary = summarize_score(label_used, pathB_rank_used)

            print(
                f"[DEBUG] {variant} {score_name}: score_len={len(pathB_raw_ensemble)}, "
                f"label_len={len(labels)}, used_len={used_len}, label_sum_used={label_sum_used}"
            )

            common_run_ids = sorted(set(final_runs) & set(pathB_runs))
            for run_id in common_run_ids:
                per_save_used_len = aligned_len(labels, final_runs[run_id], pathB_runs[run_id])
                per_save_labels = labels[:per_save_used_len]
                auc_final_save = roc_auc_score(
                    per_save_labels,
                    final_runs[run_id][:per_save_used_len],
                )
                auc_pathB_save = roc_auc_score(
                    per_save_labels,
                    pathB_runs[run_id][:per_save_used_len],
                )
            ...
            rows.append(
                [
                    ...
                    auc_pathB_raw,
                    auc_pathB_rank,
                    auc_pathB_raw - auc_final_raw
                    if not np.isnan(auc_pathB_raw) and not np.isnan(auc_final_raw)
                    else float("nan"),
                    auc_pathB_raw - auc_final_rank
                    if not np.isnan(auc_pathB_raw) and not np.isnan(auc_final_rank)
                    else float("nan"),
                    auc_pathB_rank - auc_final_rank
                    if not np.isnan(auc_pathB_rank) and not np.isnan(auc_final_rank)
                    else float("nan"),
                    len(pathB_raw_ensemble),
                    len(labels),
                    used_len,
                    label_sum_used,
                    auc_reverse,
                    max(auc_pathB_rank, auc_reverse)
                    if not np.isnan(auc_pathB_rank) and not np.isnan(auc_reverse)
                    else float("nan"),
    ...
    with open(args.out, "w", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "auc_pathB_raw_ensemble",
                "auc_pathB_rank_ensemble",
                "gain_pathB_raw_vs_final_raw",
                "gain_pathB_raw_vs_final_rank",
                "gain_pathB_rank_vs_final_rank",
                "score_len",
                "label_len",
                "used_len",
                "label_sum_used",
    ...
    if not per_save_out:
        ...
        per_save_out = f"{root}_per_save{ext or '.csv'}"
    per_save_out_dir = os.path.dirname(per_save_out)
    if per_save_out_dir:
        os.makedirs(per_save_out_dir, exist_ok=True)
    ...
    print(f"saved hidden topology AUC summary to {args.out}")
    print(f"saved hidden topology per-save AUC summary to {per_save_out}")
