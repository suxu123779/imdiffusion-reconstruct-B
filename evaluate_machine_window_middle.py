...
for iteration in os.listdir("train_result"):
    ...
    os.makedirs(f"window_result/{iteration}", exist_ok=True)

    # >>> MOD 1: 评估时应当遍历 train_result/saveX/data/ 下的子目录（而不是 saveX/）
    data_dir = f"train_result/{iteration}"
    if not os.path.isdir(data_dir):
        continue

    # >>> MOD 2: 只评估与命令行 --dataset 一致的数据集，跳过 SWAT 等其他目录
    ...
    #for subset_name in os.listdir(f"train_result/{iteration}/"):
        ...
        if args.dataset == "SMD":
            ...
            raise ValueError(f"Unknown dataset {args.dataset}")  # >>> MOD 3: 防止 silent bug
        ...
        print(">>> ckpt =", f"{base_folder}/best-model.pth")
        ...
        for temp_i in range(0,1):
            window_trick_evaluate_middle(model, train_error_loader_list, test_loader_list, nsample=30, scaler=1,
